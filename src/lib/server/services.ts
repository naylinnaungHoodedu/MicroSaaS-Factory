...
function listSelfServePlans(plans: PlatformPlan[]) {
  const visiblePlans = plans.filter((plan) => !plan.hidden);
  return visiblePlans.length > 0 ? visiblePlans : plans;
}
...
export async function getPublicPricingData() {
  ...
  return {
    ...
    plans: listSelfServePlans(database.platformPlans),
  };
}
...
export async function createSignupIntent(input: {
  ...
  return updateDatabase((database) => {
    ...
    const plan = listSelfServePlans(database.platformPlans).find(
  ...
  return updateDatabase((database) => {
    ...
    const metadata =
      typeof object.metadata === "object" && object.metadata
        ...
        : {};
    const workspaceId = String(metadata.workspaceId ?? "").trim() || undefined;
    ...
    const nextStatus =
      resolvePlatformSubscriptionStatus(String(object.status ?? object.subscription_status ?? "")) ??
      null;
    ...
    for (const subscription of matches) {
      ...
      if (nextStatus) {
        subscription.status = subscription.status === "beta" ? "beta" : nextStatus;
      }
...
export async function getAdminOverview() {
  ...
  return {
    ...
    platformPlans: database.platformPlans,
