...
export default async function AdminPage({
    ...
    return (
      <main className="page-shell py-10">
        ...
        >
          ...
          <form action={adminLoginAction} className="max-w-xl space-y-5 rounded-[1.5rem] border border-white/10 bg-white/5 p-6">
  ...
  const plansById = new Map(overview.platformPlans.map((plan) => [plan.id, plan]));
  const latestRuns = [
    {
      key: "live-ops",
      ...
      action: runLiveOpsAutomationAction,
    ...
    },
    {
      ...
      action: runValidationCrmAutomationAction,
    ...
    <main className="page-shell py-10 space-y-8">
      ...
      >
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          <form action={createInviteAction} className="space-y-5 rounded-[1.5rem] border border-white/10 bg-white/5 p-6">
          ...
          <form action={updateFeatureFlagsAction} className="space-y-4 rounded-[1.5rem] border border-white/10 bg-slate-950/55 p-6">
            ...
            {[
              ...
              <label key={name} className="flex items-center gap-3 rounded-2xl border border-white/10 px-4 py-3 text-sm text-slate-200">
                <input
                  name={name}
                  type="checkbox"
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[0.92fr_1.08fr]">
          <div className="space-y-6 rounded-[1.5rem] border border-white/10 bg-slate-950/55 p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <StatusPill
                status={
                  overview.automation.internalKeyConfigured ? "connected" : "warning"
                }
              />
            </div>
          ...
          <div className="grid gap-6">
            {latestRuns.map((entry) => (
              <div
                key={entry.key}
                className="rounded-[1.5rem] border border-white/10 bg-slate-950/55 p-6"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-400">
            ...
            {overview.automation.recentRuns.length > 0 ? (
              overview.automation.recentRuns.map((run) => (
                ...
                >
                  ...
                  {run.error ? (
                    <p className="mt-4 text-sm text-rose-200">{run.error}</p>
                  ) : null}
                </div>
              ))
            ) : (
        ...
        <div className="space-y-4">
          {overview.signupIntents.length > 0 ? (
            overview.signupIntents.map((intent) => (
              <div
                key={intent.id}
                className="rounded-[1.4rem] border border-white/10 bg-slate-950/55 p-5 text-sm text-slate-300"
              >
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-slate-400">
                      Plan: {plansById.get(intent.planId)?.name ?? intent.planId}
                    </p>
